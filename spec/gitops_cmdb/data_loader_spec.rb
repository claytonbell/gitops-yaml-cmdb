describe GitopsCmdb::DataLoader do
  ...
  context 'read and parse a yaml file from the local filesystem' do 
    ...
    it 'by uri file://' do
      expect(subject.uri('file://spec/fixtures/valid.yaml')).to eq(data)
    end
  ...
  context 'raises error when' do 
    it 'file not found' do
      expect { subject.uri('file://spec/file_not_exist.yaml') }.to raise_error(Errno::ENOENT)
    end

    it 'can not parse yaml file' do
      expect { subject.uri('file://spec/fixtures/invalid.yaml') }.to raise_error(Psych::SyntaxError)
    end

    it 'file name extension is not .yml or .yaml' do
      expect { subject.uri('file://file.json') }.to raise_error(error)
    end

    it 'uri schema is not file://' do 
      expect { subject.uri('https://example.com/file.yaml') }.to raise_error(error)
    end
  end
